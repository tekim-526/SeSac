...
class AppDelegate: UIResponder, UIApplicationDelegate{
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let config = Realm.Configuration(schemaVersion: 3) { migration, oldSchemaVersion in
            ...
            if oldSchemaVersion < 3 { // DetailTodo deadline Column 추가
...
//        let config = Realm.Configuration(schemaVersion: 2, deleteRealmIfMigrationNeeded: true)
        ...
        let config = Realm.Configuration(schemaVersion: 8) { migration, oldSchemaVersion in
            // 컬럼 단순 추가 삭제의 경우엔 별도 코드 필요 없음
            if oldSchemaVersion < 1 { }
            if oldSchemaVersion < 2 { }
            
            if oldSchemaVersion < 3 {
                migration.renameProperty(onType: Todo.className(), from: "favorite2", to: "favorite")
            }
            if oldSchemaVersion < 4 {
                migration.enumerateObjects(ofType: Todo.className()) { oldObject, newObject in
                    guard let new = newObject else {return}
                    guard let old = oldObject else {return}
                    new["userDescription"] = "안녕하세요 \(old["title"]!) + \(old["favorite"]!)"
                }
            }
